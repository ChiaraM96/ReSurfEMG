...
def percentile_snr(env, window_length=20480):
    ...
    q1 = env_series.rolling(window=window_length, min_periods=1, center=True).quantile(
        0.25
    )
    q3 = env_series.rolling(window=window_length, min_periods=1, center=True).quantile(
        0.75
    )
    if np.any(q1 == 0):
        msg = (
            "Warning: The 25th percentile of the EMG envelope is zero in "
